...
class UNet(nn.Module):
    """
    n_channels: number of input image channel
    n_classes: class for prediction
    bilinear: whether use deconvolution
    """
...
class PspNet(nn.Module):
    """
    num_class: class of prediction
    pretrained: whether use pre-train parameter for base model
    use_aux: True use branch to accelerate the training
    """
    def __init__(self, num_class, pretrained=True, use_aux=False):
        ...
        self.ppm = PyramidPoolingModule(2048, 512, (1, 2, 3, 6), lstm=False)
