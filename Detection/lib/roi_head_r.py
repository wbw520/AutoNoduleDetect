...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self, class_logits, box_regression, proposals, image_shapes, boxes_feature):
        ...
        pred_feature = boxes_feature.split(boxes_per_image, 0)

        all_boxes = []
        all_scores = []
        all_labels = []
        all_F = []
        for boxes, scores, image_shape, features in zip(pred_boxes, pred_scores, image_shapes, pred_feature):
            ...
            boxes, scores, labels, FS = boxes[keep], scores[keep], labels[keep], FS[keep]
            ...
            all_F.append(FS)
        return all_boxes, all_scores, all_labels, all_F
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        if self.training:
            ...
            boxes, scores, labels, FS = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes, box_features)
